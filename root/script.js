...
function outoFill() {
    ...
    if (userExist) {
        ...
        const remmemberMe = parseObj.remmemberMe;
        if(remmemberMe){
            document.getElementById("theUserPasswordId").value = parseObj.password ;
        }
...
function login() {
    ...
    const remmemberMe = document.getElementById('remember').checked;
    loginUser = userName;
...
function checkForMatch() {
    if (flippedCards[0].querySelector('.card-back').textContent === flippedCards[1].querySelector('.card-back').textContent) {
        ...
        if (matchedPairs === pairsNum) {
            ...
            textAnounce.innerText = "! גאון "
            const icons = document.createElement("div");
            icons.classList.add('icons');

            // const restart = document.createElement("div");
            // restart.classList.add('restart');
            // restart.innerText = "שחק שוב";
            ...
            // icons.appendChild(restart);
            // icons.appendChild(nextLevel);
